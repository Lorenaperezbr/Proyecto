...
# http://dgrn.mgap.gub.uy/Metadatos/ine_ccz_mvd_metadatos.pdf
...
marzo <- conteo_mar[velocidad_mar, 
                    on = c('cod_detector', 'id_carril', 'fecha', 'hora', 'latitud', 'longitud'), nomatch = 0]
# Fede: Les cambie la forma de hacer join para que sea más rápido.
# En realidad, podrían hacer todo de una.
...
# Fede: Revisen el sistema de coordenadas creo que esta bien, pero revisen.
municipios_sf <- st_read("ine_ccz_mvd.shp")
st_crs(x = municipios_sf) <-  5382 #4326
municipios_sf <- st_transform(municipios_sf, "+init=epsg:4326")
febrero_marzo_sf <- st_as_sf(febrero_marzo[sample(1:.N, 0.01*.N), ], coords = c("longitud", "latitud")) # muestra para probar más rápido. Me parece que les va a explotar con todos los datos.
# febrero_marzo_sf <- st_as_sf(febrero_marzo, coords = c("longitud", "latitud")) # Todito
# Asignar el mismo sistema (asumiendo que los datos de feb-mar están en ese, chequear por las dudas.)
st_crs(febrero_marzo_sf) <- st_crs(municipios_sf)
...
# Join
base <- 
  st_join(febrero_marzo_sf, municipios_sf) %>% # Por defecto st_intersects, se puede poner left...
  filter(!is.na(velocidad_promedio)) # Sacar posibles missing

# base2 <-
#   st_join(municipios_sf, febrero_marzo_sf) %>%
#   filter(!is.na(velocidad_promedio))

# ggplot() +
#   geom_sf(data = municipios_sf) + 
#   geom_sf(data = base2, aes(fill = velocidad_promedio))


# Graficamos de nuevo (es lo mismo, tiene sentido que no queden observaciones afuera si los censores están en mdeo)
ggplot() + 
  geom_sf(data = municipios_sf) + 
  geom_sf(data = base) + 
  theme_minimal()
...
ggplot() + 
  geom_sf(data = municipios_sf) + 
  geom_sf(data = base, aes(fill=velocidad_promedio)) 

# La clase sf tiene el metodo plot.
# plot(base2["velocidad_promedio"])
# Más ejemplos: https://cengel.github.io/R-spatial/mapping.html
