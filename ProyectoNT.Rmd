...
output: 
  ...
  html_document:
  df_print: paged
header-includes: \usepackage{float} \floatplacement{figure}{H} \usepackage[spanish,es-tabla,es-nodecimaldot]{babel} \usepackage[utf8]{inputenc} \usepackage{booktabs}
---
...
En este sentido, la Figura 2 efectivamente se alinea a lo hipotetizado anteriormente donde no parece existir diferencias importantes entre los días de la semana, siendo los fines de semana aquellos días donde la relación entre velocidad y conteo mantiene una mayor correlación positiva.

Análogamente a lo anterior, la Figura 3 muestra que en términos del momento del mes la relación de asociación encontrada es positiva y no varía de forma sustancial a traves de las franjas diarias durante los meses. 
...
velocidades<-velocidades[,-6]

colnames(velocidades) <- c("CCZ", "Velocidad mínima", "Velocidad Máxima", "Volumen mínimo", "Volumen máximo")

velocidades %>%
  kable(booktabs = T,
        caption = "Tabla de máximos y mínimos de volumen y velocidad promedio") %>%
    kable_styling(latex_options = c("striped", "hold_position"))
```



En línea al estudio localizando las observaciones según CCZ, la Figura 6 muestra la distribución de la velocidad promedio y el conteo vehicular distinguiendo según CCZ a partir de un diagrama de caja ordenado según la mediana de los datos según CCZ. Vale recordar que la caja representa al rango intercuartílico (RI) de la distribución, los bigotes superiores e inferiores representan 1.5 veces el RI y los puntos por fuera, datos considerados atípicos. 
...
Sin embargo, así como no se observan sustanciales diferencias en cuanto al conteo vehicular a lo largo del mes, cuando se observa a traves de los días de la semana, como es de esperar, la Figura 8 muestra que durante los fines de semana, la movilidad se reduce a lo largo de todos los CCZ. Esta observación es más clara en las zonas céntricas de mayor movilidad asociado a que son centros laborales que tienen picos de movilidad durante la semana y valles sobre Sábados y Domingos.
...
Se puede acceder a la aplicación a partir del sigueinte link: https://martinoliverab.shinyapps.io/proyecto/
