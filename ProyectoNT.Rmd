...
base$cat_hora <- ifelse(base$hora >= "8H 00M 0S" & base$hora < "12H 00M 0S",
                           "Mañana",
...
base$cat_fecha <- ifelse(
  (base$fecha >= "2021-02-01" & base$fecha <= "2021-02-10") | (base$fecha >= "2021-03-01" & base$fecha <= "2021-03-10"),
  "Principio de mes",
  ifelse((base$fecha >= "2021-02-11" & base$fecha <= "2021-02-20") | (base$fecha >= "2021-03-11" &  base$fecha <= "2021-03-20"),
         "Mitad de mes",
         "Fin de mes"
         )
  )
...
base <- subset(base, select= -c(fecha, hora, dsc_avenida , dsc_int_anterior ,  dsc_int_siguiente ,  i.dsc_avenida, i.dsc_int_anterior,  i.dsc_int_siguiente))
...
```{r boxplot}
base %>% ggplot(aes(x=volume,y=velocidad_promedio)) +
...
base %>% 
  ...
  ggplot(aes(x = cat_fecha, y=promedio_vel)) +
...
ggpairs(base, c("volume", "velocidad_promedio"), mapping = ggplot2::aes(color = cat_hora, alpha = 0.5),
        diag = list(continuous = wrap("densityDiag")),
        lower = list(continuous = wrap("points", alpha=0.9)))

ggpairs(base, c("volume", "velocidad_promedio"), mapping = ggplot2::aes(color = dia_semana, alpha = 0.5),
        diag = list(continuous = wrap("densityDiag")),
        lower = list(continuous = wrap("points", alpha=0.9)))

ggpairs(base, c("volume", "velocidad_promedio"), mapping = ggplot2::aes(color = cat_fecha, alpha = 0.5),
        diag = list(continuous = wrap("densityDiag")),
        lower = list(continuous = wrap("points", alpha=0.9)))
```
